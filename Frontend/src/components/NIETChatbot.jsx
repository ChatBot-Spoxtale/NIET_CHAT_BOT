    ...
    <>
      {!open && (
        <button
          ...
          className="fixed bottom-6 right-6 z-[100] w-14 h-14 rounded-2xl bg-gradient-to-br from-[#e2111f] to-[#b00d18] flex items-center justify-center  transition-all duration-300 hover:scale-110 active:scale-95 group overflow-hidden"
      ...
      {open && (
        <div className="fixed inset-0 z-[90] pointer-events-none flex flex-col items-end justify-end p-2 sm:p-6 animate-in fade-in duration-300">
          <div className="relative pointer-events-auto w-full sm:w-[380px] md:w-[480px] lg:w-[560px] h-full sm:h-[min(700px,calc(100vh-120px))] md:h-[min(800px,calc(100vh-100px))] lg:h-[min(850px,calc(100vh-80px))] bg-white border border-slate-100  flex flex-col animate-in slide-in-from-bottom-8 duration-500 rounded-[32px] transform-gpu">
            {/* Desktop Close Button */}
            <button
              onClick={() => setOpen(false)}
              className="absolute -top-3 -right-3 z-[101] w-7 h-7 rounded-full bg-[#e2111f] items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 hidden sm:flex border-2 border-white"
